...
namespace MaHoa
{
    public class PlayFair
    {
        ...
        /// <param name="isEncrypt">Default this is method for encrypt</param>
        /// <returns></returns>
#if TEST
        public string HoanDoi(Coordinate dau, Coordinate cuoi, bool isEncrypt = true)
#else
        private string HoanDoi(Coordinate dau, Coordinate cuoi,bool isEncrypt = true)
#endif
        {
            int value = 1;
            if (!isEncrypt)
...
#else
        private string DecryptTwoCharacter(char one, char two)
#endif
        {
            if (one == ' ' || two == ' ')
                throw new ArgumentException();
